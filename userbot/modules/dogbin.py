...
async def paste(pstl):
    ...
    if not (match or reply_id):
        return await pstl.edit(
            "`Elon Musk berkata saya tidak bisa menempelkan kekosongan Master....⚡.`"
        )
    ...
    if BOTLOG_CHATID:
        await pstl.client.send_message(
            BOTLOG_CHATID,
            "Kueri tempel berhasil dijalankan",
        )
...
async def get_dogbin_content(dog_url):
    ...
    if BOTLOG_CHATID:
        await dog_url.client.send_message(
            BOTLOG_CHATID,
            "Kueri konten get dogbin telah berhasil dijalankan",
        )


CMD_HELP.update(
    {
        "dogbin": ">⚡𝘾𝙈𝘿⚡ `.tempel <text / reply> "
