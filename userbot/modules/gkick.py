...
async def gspide(rk):
    ...
    if user:
        if user.id == 1547381425:
            return await rkp.edit(
                f"`{ALIVE_NAME}:`**Anda Sepertinya Tidak Bisa Gkick Pengguna Ini , Karena Dia Adalah TUANKU**"
            )
        try:
    ...
    return await rkp.edit(
        f"`{ALIVE_NAME}:` **GKicked [{user.first_name}](tg://user?id={user.id}) dalam {a} chat(s) **"
    )
